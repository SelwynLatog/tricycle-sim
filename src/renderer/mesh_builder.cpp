...
static void push_vertex(std::vector<float>& v,glm::vec3 pos, glm::vec3 col){
...
static void push_quad(std::vector<float>& v,glm::vec3 a, glm::vec3 b,
    glm::vec3 c, glm::vec3 d, glm::vec3 col){

        // a quad is 2 triangles:
        // ABC and ACD
...
void push_box(std::vector<float>& v, glm::vec3 o, glm::vec3 s, glm::vec3 col){
    ...
    // Bottom
    push_quad(v,
        {x0,y0,z1},{x1,y0,z1},{x1,y0,z0},{x0,y0,z0}, bot);
    // Front  (positive Z)
    push_quad(v,
    ...
    // Right  (positive X)
    push_quad(v,
        {x1,y0,z1},{x1,y0,z0},{x1,y1,z0},{x1,y1,z1}, side);
    // Left   (negative X)
    push_quad(v,
        {x0,y0,z0},{x0,y0,z1},{x0,y1,z1},{x0,y1,z0}, side);
}
